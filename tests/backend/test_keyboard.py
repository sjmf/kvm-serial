...
class TestKeyboard:
    ...
    def test_run_keyboard_selects_correct_handler_and_runs(
        ...
        try:
            ...
            mock_pyusbop.PyUSBOp.assert_called_once_with(mock_serial, layout="en_GB", comm_cls=None)
            mock_pyusbop.PyUSBOp.return_value.run.assert_called_once()

            # PYNPUT mode
            listener = KeyboardListener(mock_serial, mode=Mode.PYNPUT)
            listener.run_keyboard()
            mock_pynputop.PynputOp.assert_called_once_with(
                mock_serial, layout="en_GB", comm_cls=None
            )
            ...
            listener.run_keyboard()
            mock_ttyop.TtyOp.assert_called_once_with(mock_serial, layout="en_GB", comm_cls=None)
            mock_ttyop.TtyOp.return_value.run.assert_called_once()

            # CURSES mode
            listener = KeyboardListener(mock_serial, mode=Mode.CURSES)
            listener.run_keyboard()
            mock_cursesop.CursesOp.assert_called_once_with(
                mock_serial, layout="en_GB", comm_cls=None
            )
